...
from ..services.ota_uploader import run_ota_with_callback
...
def _sync_ports_from_capabilities(managed_device: ManagedDevice, capabilities: Optional[Dict[str, Any]]):
    ...
    for port in managed_device.ir_ports:
        ...
        if entry:
            ...
            if description and not port.connected_device_name:
                port.connected_device_name = description
        else:
            port.is_active = False
            port.connected_device_name = None
...
def _build_capabilities_payload_lines(
    ...
    *,
    ...
    if library_ids:
        capabilities["library_ids"] = sorted(library_ids)
...
def _build_button_section(port_profiles: List[PortProfile]) -> str:
    ...
    if not assigned:
        return "# Button actions (auto-generated)"
...
def _render_dynamic_yaml(
    ...
    port_script_lines: List[str] = []

    for profile in port_profiles:
        if profile.port_number in (1, 2, 3, 4, 5):
            port_script_lines.extend(_build_port_command_scripts(profile))

    all_script_lines = shared_script_lines + port_script_lines
    script_section = "\n".join(_indent(all_script_lines, 2)).rstrip()
    ...
    async def event_stream():
        ...
        try:
            ...
            for hostname in payload.hostnames:
                ...
                ota_password = settings_service.get_setting("esphome_ota_password", "") or ""
