...
class ESPHomeClient:
    ...
    async def get_capabilities_snapshot(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            unsubscribe = self.client.subscribe_states(state_callback)

            report_service = next((svc for svc in services if svc.name == "report_capabilities"), None)
            if report_service:
                self.client.execute_service(report_service, {})
            else:
...
class ESPHomeManager:
    ...
    async def fetch_capabilities(self, hostname: str, ip_address: str) -> Optional[Dict[str, Any]]:
    ...
    async def send_tv_command(self, hostname: str, ip_address: str, command: str, box: int = 0, **kwargs) -> bool:
        ...
        # Add box parameter if specified (for multi-box setups)
        if box > 0:
            service_data["box"] = box

        if command == "channel":
            if "channel" in kwargs:
                # Format channel for multi-box setups if box is specified
                if box > 0:
                    service_data["channel"] = f"{box}-{kwargs['channel']}"
                else:
