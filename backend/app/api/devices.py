...
class CommandRequest(BaseModel):
    ...
    box: Optional[int] = 0  # For Foxtel devices
...
async def stop_discovery():
    ...
    return {"message": "Device discovery stopped"}
...
async def register_device(hostname: str, device_update: DeviceUpdate, db: Session = Depends(get_db)):
    ...
    device = Device(
        ...
        is_online=True,
        capabilities={"outputs": 5 if discovered_device.device_type == "foxtel" else 1}
    )
...
async def send_command(hostname: str, command_request: CommandRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if device.device_type == "foxtel":
            success = await esphome_manager.send_foxtel_command(
