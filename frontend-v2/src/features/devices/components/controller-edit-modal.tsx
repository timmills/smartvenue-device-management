...
const ManagedControllerModal = ({ device, open, onClose, onSaved }: ManagedModalProps) => {
  ...
  const [venueName, setVenueName] = useState(device.venue_name ?? '');
  ...
  useEffect(() => {
    ...
    setVenueName(device.venue_name ?? '');
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const payload: ManagedDeviceUpdatePayload = {
      ...
      venue_name: sanitizeValue(venueName),
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div
        role="dialog"
        aria-modal="true"
        className="relative z-10 h-[90vh] w-full max-w-3xl overflow-hidden rounded-xl bg-white shadow-xl"
      >
...
function resolveDefaultChannelSource(value: string | null): DefaultChannelSource {
  ...
  return value.startsWith('channel:') ? 'option' : 'custom';
}
